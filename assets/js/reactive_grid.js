...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function render(time) {
        ...
        for (let [el, data] of activeTargets.entries()) {
            ...
            const radius = Math.max(300, rect.width * 0.7);
            
            // Minor grid glow
            let grad = ctx.createRadialGradient(x, y, 0, x, y, radius);
            grad.addColorStop(0, `rgba(0, 255, 64, ${0.4 * data.intensity})`);
            grad.addColorStop(1, 'rgba(0, 255, 64, 0)');
            ...
            gradMajor.addColorStop(0, `rgba(0, 255, 64, ${0.7 * data.intensity})`);
            gradMajor.addColorStop(1, 'rgba(0, 255, 64, 0)');
